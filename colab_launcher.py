...
def install_launcher_deps() -> None:
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "-q", 
         "openai>=1.0.0", "requests", "duckduckgo-search", "tavily-python"],
        check=True,
    )
...
MODEL_MAIN = get_cfg("OUROBOROS_MODEL", default="google/gemini-2.5-pro")
MODEL_CODE = get_cfg("OUROBOROS_MODEL_CODE", default="Qwen3-Coder-Plus")
MODEL_LIGHT = get_cfg("OUROBOROS_MODEL_LIGHT", default="google/gemini-2.5-flash-lite")
...
os.environ["TAVILY_API_KEY"] = str(TAVILY_API_KEY or "")
