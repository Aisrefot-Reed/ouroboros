...
def init_state() -> Dict[str, Any]:
    ...
    In FlowAI-only mode we do not call external billing APIs. Budget drift is
    tracked purely from our own events.jsonl usage.
    """
    lock_fd = acquire_file_lock(STATE_LOCK_PATH)
    try:
        st = _load_state_unlocked()

        # Capture session snapshots for drift detection (self-tracked only)
        st["session_spent_snapshot"] = float(st.get("spent_usd") or 0.0)
        # We no longer fetch external billing ground truth; use 0 as baseline.
...
def check_openrouter_ground_truth() -> Optional[Dict[str, float]]:
    """
    Disabled in FlowAI-only mode.

    Kept for backwards compatibility but always returns None so that budget
    drift is computed only from our own accounting.
    """
    return None
...
def update_budget_from_usage(usage: Dict[str, Any]) -> None:
    """Update state with LLM usage costs and tokens (self-tracked only).
